...
searchRunBtn.addEventListener("click", async () => {
  ...
  try {
    if (!API_BASE_URL) {
      searchFeedback.textContent = "Erreur: API_BASE_URL manquante dans config.js";
      return;
    }
    ...
    try {
      ...
      searchFeedback.textContent = "Erreur: reponse non JSON du proxy.";
      return;
    }
    ...
    if (String(error.message || "").includes("Failed to fetch")) {
      searchFeedback.textContent = "Erreur: Failed to fetch (CORS ou blocage Cloudflare).";
    } else {
