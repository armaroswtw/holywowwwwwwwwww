exports.handler = async (event) => {
  ...
  try {
    ...
    if (!type || !value) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: 'Missing type or value' })
      };
    }

    // Construire l'URL pour le reverse lookup
    const endpoints = {
      email: 'lookup/email',
      phone: 'lookup/phone',
      iban: 'lookup/iban'
    ...
    if (!endpoint) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: 'Invalid lookup type' })
      };
    }

    // Faire l'appel à l'API BrixHub
    const response = await fetch(`${BASE_URL}/${endpoint}/${encodeURIComponent(value)}`, {
      method: 'GET',
      headers: {
        'X-API-Key': API_KEY,
    ...
    // Retourner la réponse avec les headers de rate limit
    return {
